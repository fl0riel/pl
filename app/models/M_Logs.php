...
class M_Logs extends Model
{
    ...
    public function getFileLogs(int $start, int $end)
    {
        $db = DB::getPdoLow();
        try {
            $sql = $db->prepare('SELECT id, updated_by, action, path, new_path, date_format(action_time, "%d/%m/%Y") FROM file_log ORDER BY action_time DESC LIMIT :firstResult, :lastResult');
