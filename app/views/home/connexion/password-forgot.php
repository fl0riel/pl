<body class="login">
    <form action="/www/stage/plateforme_partage/app/views/home/traitement/password-forgot.php" method="POST" class="login__form">
        <fieldset>
            ...
            <label class="login__label label" for="email">
                <input class="login__input" type="email" name="email" id="email" placeholder="Votre Email" pattern="^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,5})$" maxlength="50" <?php if(isset($data['confirmation'])){ echo ' disabled';} ?>>
                <?php if(isset($data['confirmation'])){ echo '<span class="label__text"></span>';} else {
                    echo '<span class="label__text">Email</span>';
                } ?>
            </label>
            <?php 
            if(isset($data['confirmation'])) {
                echo '<a href="/www/stage/plateforme_partage/public/home/connect" class="login__link">Se connecter a nouveau</a>';
